...
const Expenses = () => {
    ...
    const aggregatedData: AggregatedDataItem[] = useMemo(() => {
        const filtered: AggregatedData = expenses.filter((expense: ExpenseByCategorySummary) =>{
            const matchesCategory = selectedCategory === "All" || expense.category === selectedCategory;
            const expenseDate = parseDate(expense.date);
            const matchesDate = !startDate || !endDate || (expenseDate >= startDate && expenseDate <= endDate);
            return matchesCategory && matchesDate
        }).reduce((acc: AggregatedData, expense: ExpenseByCategorySummary) => {
            const amount = parseInt(expense.amount);
            if(!acc[expense.category]){
                acc[expense.category] = {name: expense.category, amount: 0};
                acc[expense.category].color = `#${Math.floor(Math.random() * 16777215)
                    .toString(16)}`;
                acc[expense.category].amount += amount;
            }
            return acc;
        }, {});
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4">
            ...
            <div className="w-full md:w-1/3 bg-white shadow rounded-lg p-6">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="category" id="category" className={classNames.selectInput} defaultValue="All"
                    ...
                    <div>
                        ...
                        <input type="date" id="start-date" name="start-date" className={classNames.selectInput} 
                        onChange={(e)=>{
                    ...
                    <div>
                        ...
                        <input type="date" id="end-date" name="end-date" className={classNames.selectInput} 
            ...
            <div className="flex-grow bg-white shadow rounded-lg-p-4 md:p-6">
                <ResponsiveContainer width="100%" height={400}>
                    <PieChart>
                        <Pie data={aggregatedData} cx="50%" cy="50%" label outerRadius={150}
                        fill="#8884d8" dataKey="amount" onMouseEnter={(_,index)=>setActiveIndex(index)}>
