...
const Favourite = ({setActiveIcon}) => {

    const {theme, HoverableItem} = useTheme()
    const {chats, user, identifier, formatMessageTime, getUnreadCount, 
      setSelectedChat
    } = useAppContext() 
    ...
    const favouriteChats = chats.filter(chat=>chat.favourite.some(
      data => data.user === user._id
    ))
    // console.log("fav chat: ", favouriteChats)
      ...
      <div className="w-full h-full flex flex-col p-4  rounded-tl">
        ...
        <div className="h-fit p-1 pb-2 mb-1  border-[#202c33] rounded-tl">
          <div className="flex space-x-6 items-center">
            ...
            <p className="font-semibold text-xl">Favourite</p>
          </div>
          ...
          <div className={`mt-5 mb-2 flex items-center ${theme.main} border-b ${theme.border} space-x-2 pl-2`}>
            ...
            <input
              ...
              className="w-full px-2 py-1 border-0 outline-none rounded-md"
            />
          </div>
        </div>
